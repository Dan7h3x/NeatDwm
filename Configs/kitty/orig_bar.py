...
from .fast_data_types import (DECAWM, Color, Region, Screen,
                              cell_size_for_window, get_boss, get_options,
                              pt_to_px, set_tab_bar_render_data,
                              update_tab_bar_edge_colors, viewport_for_window,
                              wcswidth)
...
class ColorFormatter:
    ...
    def __getattr__(self, name: str) -> str:
        ...
        if q == "default":
            ...
            col = color_from_int(
                (self.draw_data.tab_bg if self.which == "4" else self.draw_data.tab_fg)(
                    self.tab_data
                )
            )
...
def draw_title(
    ...
    try:
        title = eval(
            compile_template(template), {"__builtins__": safe_builtins}, eval_locals
        )
    except Exception as e:
        report_template_failure(template, str(e))
        title = tab.title
    ...
    def update(self, data: Sequence[TabBarData]) -> None:
        ...
        def draw_tab(
            ...
            s.cursor.bold, s.cursor.italic = (
                self.active_font_style if t.is_active else self.inactive_font_style
            )
            before = s.cursor.x
            end = self.draw_func(
                self.draw_data, s, t, before, max_tab_length, i + 1, t is last_tab, ed
            )
        ...
        if extra > 0:
            if ideal_tab_lengths[active_idx] > max_tab_lengths[active_idx]:
                d = min(
                    extra, ideal_tab_lengths[active_idx] - max_tab_lengths[active_idx]
                )
